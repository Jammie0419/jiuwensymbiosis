...
def _edge(front_x_mm):
    ...
    return {
        ...
        "center_mm": [front_x_mm + 100.0, 0.0, 0.0],
        "surface_z_mm": 600.0,
        "yaw_rad": 0.0,
        "long_mm": 800.0,
        "short_mm": 500.0,
...
def test_place_servo_continuous_no_per_frame_stop():
    ...
    api, env = _api(
        [
            _edge(800.0),  # approach_for_place hands off (squared + far)
            _edge(700.0),  # servo s0 → start worker
            _edge(600.0),
            _edge(500.0),  # creep, still far
            _edge(380.0),
        ]
    )  # near edge (forward 0.03 ≤ tol) → one stop
...
def test_place_servo_already_at_edge_no_creep():
    ...
    api, env = _api(
        [
            _edge(800.0),  # approach_for_place hands off (far)
            _edge(350.0),
        ]
    )  # servo s0 already at the edge → no creep
    out = api.approach_for_place("table")
    ll = env.low_level
...
def test_place_servo_disabled_uses_discrete_straight_in():
    ...
    api, env = _api(
        [
            _edge(800.0),  # far + squared → discrete straight-in step
            _edge(350.0),
        ],  # in range
        enabled=False,
    )
