...
class _Env:
    def __init__(self):
        self.low_level = _LL()
        self.cfg = CruzrConfig()  # place_approach_edge_m=0.35
...
def _surf(front_x_mm, cy_mm=0.0, yaw_rad=0.0, long_mm=0.0, short_mm=0.0):
    return {
        ...
        "center_mm": [front_x_mm + 100.0, cy_mm, 0.0],
        "surface_z_mm": 600.0,
        "yaw_rad": yaw_rad,
        "long_mm": long_mm,
        "short_mm": short_mm,
...
def test_squares_to_near_edge_when_oblique_but_positioned():
    ...
    api, env = _api(
        [
            _surf(350.0, 0.0, yaw_rad=0.3, long_mm=800.0, short_mm=500.0),
...
def test_near_square_footprint_now_squares():
    ...
    api, env = _api(
        [
            _surf(350.0, 0.0, yaw_rad=0.3, long_mm=600.0, short_mm=580.0),
...
def test_squares_to_edge_normal_when_positioned():
    ...
    api, env = _api(
        [
            _surf_edge(350.0, 0.0, edge_normal=(-math.cos(theta), -math.sin(theta))),
...
def test_squares_first_then_straight_in_when_far_and_oblique():
    ...
    api, env = _api(
        [
            _surf_edge(800.0, 0.0, edge_normal=(-math.cos(theta), -math.sin(theta))),
...
def test_straight_in_uses_reserve():
    ...
    api, env = _api([_surf_edge(800.0, 0.0, edge_normal=(-1.0, 0.0)), _surf_edge(350.0, 0.0, edge_normal=(-1.0, 0.0))])
    out = api.approach_for_place("table")
    assert out["ok"] and out["status"] == "in_range"
    dx, dy, dyaw = env.low_level.nav_calls[0]
...
def test_offset_edge_midpoint_does_not_swing_base_sideways():
    ...
    api, env = _api(
        [_surf_edge(800.0, 300.0, edge_normal=(-1.0, 0.0)), _surf_edge(350.0, 300.0, edge_normal=(-1.0, 0.0))]
    )
    out = api.approach_for_place("table")
    assert out["ok"]
...
def test_large_square_turn_is_capped():
    ...
    api, env = _api(
        [
            _surf_edge(350.0, 0.0, edge_normal=(-math.cos(big), -math.sin(big))),
            _surf_edge(350.0, 0.0, edge_normal=(-1.0, 0.0)),
        ]
    )  # squared next → terminate
    out = api.approach_for_place("table")
    assert out["ok"]
...
def test_untrusted_edge_falls_back_to_footprint():
    ...
    api, env = _api(
        [
            _surf_edge(350.0, 0.0, edge_normal=(-1.0, 0.0), edge_quality=0.9, yaw_rad=0.3),
...
def test_footprint_fallback_squares_first_then_straight_in():
    ...
    api, env = _api(
        [
            _surf(800.0, 0.0, yaw_rad=0.3, long_mm=800.0, short_mm=500.0),  # far + off-square
