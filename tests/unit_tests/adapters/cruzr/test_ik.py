...
def test_ik_recovers_a_reachable_target():
    ...
    res = ik_solve_5dof(chain, q_fixed, ARM, target_pos, target_normal, tool_normal_local=(0.0, 0.0, 1.0))
...
def test_ik_unreachable_returns_not_converged():
    ...
    res = ik_solve_5dof(chain, q_fixed, ARM, np.array([5.0, 0.0, 0.0]), np.array([1.0, 0.0, 0.0]), max_iters=100)
...
def test_ik_pose_two_axis_with_tcp_roundtrip():
    ...
    res = ik_solve_pose(
        ...
        tool_approach_local=approach_local,
        tool_paddle_local=paddle_local,
...
def test_ik_pose_tcp_offset_shifts_wrist_target():
    ...
    res = ik_solve_pose(
        ...
        tool_approach_local=(0, 0, 1),
        tool_paddle_local=(1, 0, 0),
        tcp_offset_local=tcp_local,
    )
    assert res.converged
