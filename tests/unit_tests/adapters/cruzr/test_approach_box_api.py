...
class _LL:
    ...
    def navigate_relative(self, dx_m, dy_m=0.0, dyaw_rad=0.0, *, k_rot=None, k_rot_slow_rad=None, k_fwd=None):
...
def _make_cfg(**over):
    base = dict(
        ...
        approach_k_rot=1.5,
        approach_k_rot_slow_rad=0.5,
        approach_k_fwd=0.8,  # gentle approach-only gains
        ...
        grasp_arc_enabled=False,
        grasp_arc_standoff_m=0.10,
        grasp_arc_min_radius_m=0.35,
        grasp_arc_max_dyaw_rad=1.2,
        grasp_face_flatness_max=0.15,
        arc_k_fwd=0.6,
    )
    base.update(over)
...
def test_cached_detection_skips_the_search_pass(monkeypatch):
    ...
    api = CruzrApi(_Env())
    api._last_detection = _det(400.0)
    searched = []
    monkeypatch.setattr(
        approach,
        "_face_object",
...
def test_in_band_first_iter_does_not_move():
    api = CruzrApi(_Env())
    api._last_detection = _det(400.0)  # rng .4, centered → in_band
    out = api.approach_for_grasp()
...
def test_too_close_does_not_move():
    api = CruzrApi(_Env())
    api._last_detection = _det(200.0)  # rng .2 < min .3 → too_close
    out = api.approach_for_grasp()
...
def test_grounding_sticky_across_redetects():
    ...
    api._last_detection = {
        "ok": True,
        "center_mm": [700.0, 200.0, 780.0],
        "object": "white box",
        "reference": "brown table",
    }  # far + off-centre
    ons = []
...
def test_approach_drives_use_gentle_base_gains():
    ...
    assert all(
        k == {"k_rot": 1.5, "k_rot_slow_rad": 0.5, "k_fwd": 0.8} for k in api.env.low_level.kw
    )  # gentle gains on every approach drive
...
def test_footprint_squares_up_in_place_before_grasp():
    ...
    dx, _dy, dyaw = (api.env.low_level.calls[0][1], api.env.low_level.calls[0][2], api.env.low_level.calls[0][3])
...
def test_squared_straight_in_reserves_final_segment_no_per_step_chop():
    ...
    api._last_detection = {
        "ok": True,
        "center_mm": [1300.0, 0.0, 780.0],
        "object": "box",
        "yaw_rad": 0.0,
        "long_mm": 300.0,
        "short_mm": 200.0,
    }  # far (rng 1.3) + already square
    _script(
        api,
        [
            {
                "ok": True,
                "center_mm": [550.0, 0.0, 780.0],
                "object": "box",
                "yaw_rad": 0.0,
                "long_mm": 300.0,
                "short_mm": 200.0,
            },  # after the reserved move
            {
                "ok": True,
                "center_mm": [400.0, 0.0, 780.0],
                "object": "box",
...
def test_single_shot_one_final_alignment():
    ...
    api._last_detection = {
        ...
        "face_normal": [-0.866, 0.5],
        "face_flatness": 0.05,
    }  # corner-on → 2-leg route
    _script(
        api,
        [
            {
                "ok": True,
                "center_mm": [600.0, 0.0, 780.0],
                "object": "box",  # post-route: short + tilted
                "face_normal": [-0.95, 0.31],
                "face_flatness": 0.05,
            },
            {
                "ok": True,
                "center_mm": [400.0, 0.0, 780.0],
                "object": "box",  # after align → cache
                "face_normal": [-1.0, 0.0],
                "face_flatness": 0.05,
            },
        ],
...
def test_single_shot_too_close_after_route_aborts():
    ...
    _script(
        ...
        [
            {
                "ok": True,
                "center_mm": [200.0, 0.0, 780.0],
                "object": "box",  # overshot: rng .2 < min .30
                "face_normal": [-1.0, 0.0],
                "face_flatness": 0.05,
            }
        ],
